#Author : Roche Christopher
#File created on 18 Aug 2019 10:04 AM

from mpu import MPU
import time
import RPi.GPIO as GPIO

# variables
# device address
device_address = 0X68

mpu6050 = MPU(device_address)

#gyro related variables

mpu6050.initialize(gyro_config=int('00010000',2), smplrt_div_value = 0, general_config=1)
mpu6050.set_interrupt_pin_configuration(int('10100000', 2))
gyro_rate = 0.001 # 1 ms
gyro_scale = 32.8
is_data_available = False

PIN = 18 
GPIO.setmode(GPIO.BCM)
GPIO.setup(PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)

# Define the callback function for the interrupt
def gpio_falling_edge_callback(channel):
	is_data_available = False

# Attach the interrupt on the falling edge
GPIO.add_event_detect(PIN, GPIO.FALLING, callback=gpio_falling_edge_callback, bouncetime=200)

time.sleep(5)
while True:
	if is_data_available == True:
		is_data_available = False
		print('Data is available')
		mpu.get_interrupt_values()



print("calibrating gyroscope")

gyro_x_offset = 0

calibration_number_count = 1000
for i in range(calibration_number_count):
	gyro_x_offset += mpu6050.get_gyro_x()
	gyro_y_offset += mpu6050.get_gyro_y()
	gyro_z_offset += mpu6050.get_gyro_z()
	# wait until data is available 
	time.sleep(0.001)

gyro_x_offset /= calibration_number_count
gyro_y_offset /= calibration_number_count
gyro_z_offset /= calibration_number_count

print('gyroscope offsets x, y, z ', gyro_x_offset, gyro_y_offset, gyro_z_offset)

prev_gyro_x = mpu6050.get_gyro_x()
prev_gyro_y = mpu6050.get_gyro_y()
prev_gyro_z = mpu6050.get_gyro_z()

angle_x = 0
angle_y = 0
angle_z = 0
p_time = time.time()
while True:
	gyro_x = mpu6050.get_gyro_x() - gyro_x_offset
	gyro_y = mpu6050.get_gyro_y() - gyro_y_offset
	gyro_z = mpu6050.get_gyro_z()

	angle_x_dt = int(gyro_x / gyro_to_angle_dt)
	angle_y_dt = int(gyro_y / gyro_to_angle_dt)
	angle_z_dt = int(gyro_z / gyro_to_angle_dt)

	angle_x = angle_x + (angle_x_dt * dt)
	angle_y = angle_y + (angle_y_dt * dt)
	print(angle_x, angle_y)

	#print("before loop" , time.time() - p_time)
	#time.sleep(dt)
	while (time.time() - p_time ) < dt:
		pass
	#print(time.time() - p_time) 
	p_time = time.time()
